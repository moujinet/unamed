...
<template>
  <Teleport to="body">
    <TransitionSlide :offset="[0, 20]">
      <div
        v-if="visible"
        class="absolute bottom-0 left-0 border-(1 base) sm:bottom-auto sm:left-50% sm:top-50% sm:translate-x--1/2 sm:translate-y--1/2"
        z-110 w-full select-none rounded-t bg-sider shadow-lg transition-all sm:w-500px sm:rounded
      >
        <header
          v-if="header && title"
        ...
        >
          <div flex="~ v-center" gap-x-4 leading-none text-caption>
            <slot name="header">
              ...
              <strong text-18px font-semibold>{{ title }}</strong>
            </slot>
          </div>
